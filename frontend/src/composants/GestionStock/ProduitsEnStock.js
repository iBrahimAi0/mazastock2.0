...
const ProduitsEnStock = () => {
  ...
  const loadData = async () => {
    ...
    setProduits(
      productsResponse.data.filter(
        (product) =>
          !stockResponse.data.some((stock) => stock.produit_id === product.id),
      ),
    );
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await axios.put(
          `http://localhost:8000/api/produits_en_stock/${currentId}`,
          data,
        );
        setAlert({
          show: true,
          message: "Produit en stock mis à jour avec succès.",
          type: "success",
        });
        ...
        setAlert({
          show: true,
          message: "Produit ajouté en stock avec succès.",
          type: "success",
        });
        setToastSeverity("success");
      ...
      console.error(
        "Erreur lors de l'ajout/mise à jour du produit en stock",
        error,
      );
      const errorMessage =
  ...
  const handleDelete = async (id) => {
    if (
      window.confirm("Êtes-vous sûr de vouloir supprimer ce produit du stock ?")
    ) {
      try {
        await axios.delete(`http://localhost:8000/api/produits_en_stock/${id}`);
        setAlert({
          show: true,
          message: "Produit en stock supprimé avec succès.",
          type: "success",
        });
        loadData();
        setToastSeverity("success");
        ...
        console.error(
          "Erreur lors de la suppression du produit en stock",
          error,
        );
        setAlert({
          show: true,
          ...
          <div className="p-fluid">
            ...
            <div className="p-field">
              ...
              <InputNumber
                ...
                onValueChange={(e) => setQuantiteInitiale(e.value)}
                required
              />
            </div>
            <div className="p-field">
              ...
              <InputNumber
                ...
                onValueChange={(e) => setQuantiteActuelle(e.value)}
                required
              />
            </div>
            <div className="p-field">
              ...
              <InputNumber
                id="prixVente"
                value={prixVente}
                onValueChange={(e) => setPrixVente(e.value)}
                mode="currency"
                currency="MAD"
                locale="fr-FR"
                required

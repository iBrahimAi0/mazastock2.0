  ...
  const filteredCommandes = useMemo(
    () =>
      commandes.filter((commande) => {
        ...
        const clientName =
          `${commande.client?.nom || ""} ${commande.client?.prenom || ""}`.toLowerCase();
        if (
          searchTerm &&
  ...
  const handleChangeEtat = async (commandeId, newEtat) => {
    try {
      ...
      const updatedCommandes = commandes.map((commande) =>
        commande.id === commandeId ? { ...commande, etat: newEtat } : commande,
      );
    ...
    <div>
      ...
      <Row xs={1} md={4} className="g-4 mb-4 " role="button">
        {statCards.map((card, idx) => (
          <Col key={idx}>
            <Card
              className={`text-center shadow h-100 text-white bg-${card.color}`}
            >
              <Card.Body onClick={() => setFilter(card.state)}>
                <FontAwesomeIcon icon={card.icon} size="lg" />
                <Card.Title>{card.title}</Card.Title>
                <Card.Text>
                  {commandes.filter((c) => c.etat === card.state).length}{" "}
                  commandes
                </Card.Text>
              </Card.Body>
            </Card>
          ...
          {filteredCommandes.map((commande) => (
            <tr key={commande.id}>
              ...
              <td>
                <Dropdown>
                  <Dropdown.Toggle
                    variant={findColorForState(commande.etat)}
                    id="dropdown-basic"
                  >
                    {commande.etat}
                  </Dropdown.Toggle>
                  ...
                  <DropdownMenuPortal>
                    ...
                    >
                      <Dropdown.Item
                        onClick={() =>
                          handleChangeEtat(commande.id, "en attente")
                        }
                      >
                        En attente
                      </Dropdown.Item>
                      ...
                      <Dropdown.Item
                        onClick={() =>
                          handleChangeEtat(commande.id, "expédiée")
                        }
                      >
                        Expédiée
                      </Dropdown.Item>
              ...
              <td>
                {commande.details
                  .reduce(
                    (total, item) =>
                      total +
                      parseInt(item.quantite, 10) * parseFloat(item.prix),
                    0,
                  )
                  .toFixed(2)}{" "}
                MAD
            ...
            <Table striped bordered hover>
              <thead>
                <tr className="text-center">
                  <th>Nom Produit</th>{" "}
                  {/* Modifié de "ID Produit" à "Nom Produit" */}
                  <th>Quantité</th>
                  <th>Prix unitaire</th>
                  <th>Total</th>
                </tr>
              ...
              <tbody>
                {selectedCommande.details.map((detail) => (
                  <tr key={detail.produit_id} className="text-center">
                    ...
                    {/* Modifié pour afficher le nom du produit */}

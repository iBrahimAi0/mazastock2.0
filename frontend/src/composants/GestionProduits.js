...
const GestionProduits = () => {
  const emptyProduct = { id: null, nom: '', description: '', prix_achat: 0, categorie_id: null, fournisseur_id: null, image_produit: null };
  ...
  const handleShow = (produit) => {
    if (produit && produit.id != null) {
      ...
      setCurrentProduit({
        ...
        fournisseur_id: fournisseurs.find(f => f.id === produit.fournisseur_id),
        imagePreview: produit.image_produit ? `http://localhost:8000/storage/${produit.image_produit}` : null
      });
    } else {
      setIsEditing(false);
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append('categorie_id', currentProduit.categorie_id?.id ?? currentProduit.categorie_id);
    formData.append('fournisseur_id', currentProduit.fournisseur_id?.id ?? currentProduit.fournisseur_id);
    ...
    try {
      ...
      const errorMessage = error.response?.data?.error || error.response?.data?.message || 'Erreur lors du traitement du produit';
  ...
  const dialogFooter = (
    <div className="p-dialog-footer mt-2" style={{ display: 'flex', justifyContent: 'space-between' }}>
      ...
      <Button style={{ borderRadius: '20px' }} label={isEditing ? 'Mettre à jour' : 'Ajouter'} className="p-button-help" icon="pi pi-check" onClick={handleSubmit} />
    </div>
  );
  ...
  return (
    <div className="gestion-produits-container">
      ...
      <DataTable value={filteredProduits} responsive paginator rows={10} rowsPerPageOptions={[5, 10, 25, 50]} tableStyle={{ minWidth: '50rem' }} className="modern-datatable">
          ...
          <div className="image-upload-section">
            {currentProduit.imagePreview ? (
              <div className="image-preview-container">
                <img src={currentProduit.imagePreview} alt="Preview" className="image-preview" />
                <Button 
                  type="button"
                  label="Supprimer" 
                  icon={<FiX />}
